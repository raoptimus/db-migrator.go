...
// ─── Token / Credential masking (ФТ-10) ─────────────────────────────────────

func TestMigration_SanitizeCredentials_Token(t *testing.T) {
	tests := []struct {
		name     string
		token    string
		input    string
		...
		{
			name:     "bearer token masked in SQL",
			token:    "secret-token-abc",
			input:    "GRANT SELECT USING TOKEN secret-token-abc",
			expected: "GRANT SELECT USING TOKEN ****",
		},
		{
			name:     "token in DSN query param masked",
			token:    "mytoken",
			input:    "iceberg://host/wh?token=mytoken&other=val",
			expected: "iceberg://host/wh?token=****&other=val",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			file := NewMockFile(t)
			logger := NewMockLogger(t)
			serv := NewMigration(&Options{Token: tt.token}, logger, file, repo)
			assert.Equal(t, tt.expected, serv.SQLQueryOutput(tt.input))
		})
	}
}

func TestMigration_SanitizeCredentials_Credential(t *testing.T) {
	tests := []struct {
		name       string
		credential string
		input      string
		...
		{
			name:       "credential masked in SQL",
			credential: "client:secret",
			input:      "USE CREDENTIAL client:secret",
			expected:   "USE CREDENTIAL ****",
		},
		{
			name:       "credential in DSN query param masked",
			credential: "my-cred",
			input:      "iceberg://host/wh?credential=my-cred",
			expected:   "iceberg://host/wh?credential=****",
		},
		{
			name:       "empty credential is no-op",
			credential: "",
			input:      "SELECT 1",
			expected:   "SELECT 1",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			file := NewMockFile(t)
			logger := NewMockLogger(t)
			serv := NewMigration(&Options{Credential: tt.credential}, logger, file, repo)
			assert.Equal(t, tt.expected, serv.SQLQueryOutput(tt.input))
		})
	}
}

func TestMaskDSNParam(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		key      string
	...
	}{
		{
			...
			input:    "iceberg://h/w?token=abc123",
			key:      "token",
			expected: "iceberg://h/w?token=****",
		},
		{
			name:     "token in middle of query string",
			input:    "iceberg://h/w?a=1&token=abc123&b=2",
			key:      "token",
			expected: "iceberg://h/w?a=1&token=****&b=2",
		},
		{
			name:     "credential param",
			input:    "iceberg://h/w?credential=c:s&scope=cat",
			key:      "credential",
			expected: "iceberg://h/w?credential=****&scope=cat",
		},
		{
			name:     "no param present",
			input:    "iceberg://h/w?other=val",
			key:      "token",
			expected: "iceberg://h/w?other=val",
		},
		{
			name:     "two token occurrences both masked",
			input:    "iceberg://h/w?token=first&other=x&token=second",
			key:      "token",
			expected: "iceberg://h/w?token=****&other=x&token=****",
		},
		{
			name:     "two token occurrences at end masked",
			input:    "token=aaa&token=bbb",
			key:      "token",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := maskDSNParam(tt.input, tt.key)
...
func TestMigration_SanitizeCredentials_S3SecretAccessKey(t *testing.T) {
	tests := []struct {
		name              string
		...
		{
			name:              "bare secret access key value masked",
			s3SecretAccessKey: "mys3secret",
			input:             "CREATE TABLE t (id INT) LOCATION s3://bucket/path mys3secret",
			expected:          "CREATE TABLE t (id INT) LOCATION s3://bucket/path ****",
		},
		{
			name:              "secret access key in DSN query param masked",
			s3SecretAccessKey: "mys3secret",
			input:             "iceberg://host/wh?s3.access-key-id=ak&s3.secret-access-key=mys3secret&s3.region=us-east-1",
			expected:          "iceberg://host/wh?s3.access-key-id=ak&s3.secret-access-key=****&s3.region=us-east-1",
		},
		{
			name:              "empty secret access key is no-op",
			s3SecretAccessKey: "",
			input:             "iceberg://host/wh?s3.secret-access-key=something",
			expected:          "iceberg://host/wh?s3.secret-access-key=****",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			file := NewMockFile(t)
			logger := NewMockLogger(t)
			serv := NewMigration(&Options{S3SecretAccessKey: tt.s3SecretAccessKey}, logger, file, repo)
			assert.Equal(t, tt.expected, serv.SQLQueryOutput(tt.input))
		})
	}
}

func TestMigration_SanitizeCredentials_S3SessionToken(t *testing.T) {
	tests := []struct {
		name           string
		...
		{
			name:           "bare session token value masked",
			s3SessionToken: "sess-tok-xyz",
			input:          "CONFIGURE SESSION sess-tok-xyz FOR CATALOG",
			expected:       "CONFIGURE SESSION **** FOR CATALOG",
		},
		{
			name:           "session token in DSN query param masked",
			s3SessionToken: "sess-tok-xyz",
			input:          "iceberg://host/wh?s3.session-token=sess-tok-xyz&s3.region=us-east-1",
			expected:       "iceberg://host/wh?s3.session-token=****&s3.region=us-east-1",
		},
		{
			name:           "empty session token is no-op",
			s3SessionToken: "",
			input:          "iceberg://host/wh?s3.session-token=something",
			expected:       "iceberg://host/wh?s3.session-token=****",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			file := NewMockFile(t)
			logger := NewMockLogger(t)
			serv := NewMigration(&Options{S3SessionToken: tt.s3SessionToken}, logger, file, repo)
			assert.Equal(t, tt.expected, serv.SQLQueryOutput(tt.input))
		})
	}
}
